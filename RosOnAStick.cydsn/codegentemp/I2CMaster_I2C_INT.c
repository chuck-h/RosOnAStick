...
/*******************************************************************************
* Function Name: I2CMaster_I2C_ISR
****************************************************************************//**
*
*  Handles the Interrupt Service Routine for the SCB I2C mode.
*
*******************************************************************************/
CY_ISR(I2CMaster_I2C_ISR)
{
    uint32 diffCount;
    uint32 endTransfer;

#ifdef I2CMaster_I2C_ISR_ENTRY_CALLBACK
    I2CMaster_I2C_ISR_EntryCallback();
#endif /* I2CMaster_I2C_ISR_ENTRY_CALLBACK */

#if (I2CMaster_I2C_CUSTOM_ADDRESS_HANDLER_CONST)
    uint32 response;

    response = I2CMaster_I2C_ACK_ADDR;
    ...
    if(NULL != I2CMaster_customIntrHandler)
    {
        I2CMaster_customIntrHandler();
    }

    if(I2CMaster_CHECK_INTR_I2C_EC_MASKED(I2CMaster_INTR_I2C_EC_WAKE_UP))
    {
        /* Mask-off after wakeup */
        I2CMaster_SetI2CExtClkInterruptMode(I2CMaster_NO_INTR_SOURCES);
    }
    ...
    */
    if(I2CMaster_CHECK_I2C_FSM_MASTER)
    {
        #if(I2CMaster_I2C_MASTER)
        {
            ...
            */
            if(I2CMaster_CHECK_INTR_MASTER_MASKED(I2CMaster_INTR_MASTER_I2C_BUS_ERROR))
            {
                I2CMaster_mstrStatus |= (uint16) (I2CMaster_I2C_MSTAT_ERR_XFER |
                                                         I2CMaster_I2C_MSTAT_ERR_BUS_ERROR);

                endTransfer = I2CMaster_I2C_CMPLT_ANY_TRANSFER;
            }
            ...
            */
            if(I2CMaster_CHECK_INTR_MASTER_MASKED(I2CMaster_INTR_MASTER_I2C_ARB_LOST))
            {
                I2CMaster_mstrStatus |= (uint16) (I2CMaster_I2C_MSTAT_ERR_XFER |
                                                         I2CMaster_I2C_MSTAT_ERR_ARB_LOST);

                endTransfer = I2CMaster_I2C_CMPLT_ANY_TRANSFER;
            }

            #if(I2CMaster_I2C_MULTI_MASTER_SLAVE)
            {
                /* I2C_MASTER_CMD_M_START_ON_IDLE:
                * MultiMaster-Slave does not generate start, because Slave was addressed.
                * Pass control to slave.
                */
                if(I2CMaster_CHECK_I2C_MASTER_CMD(I2CMaster_I2C_MASTER_CMD_M_START_ON_IDLE))
                {
                    I2CMaster_mstrStatus |= (uint16) (I2CMaster_I2C_MSTAT_ERR_XFER |
                                                             I2CMaster_I2C_MSTAT_ERR_ABORT_XFER);
            ...
            */
            if(0u != endTransfer)
            {
                ...
                I2CMaster_mstrStatus |= (uint16) I2CMaster_GET_I2C_MSTAT_CMPLT;

                #if(I2CMaster_I2C_MULTI_MASTER_SLAVE)
                {
                    if(I2CMaster_CHECK_I2C_FSM_ADDR)
                    {
                        ...
                        */
                        if(I2CMaster_CHECK_I2C_MASTER_CMD(I2CMaster_I2C_MASTER_CMD_M_START_ON_IDLE))
                        {
                            I2CMaster_I2C_MASTER_CLEAR_START;

                            endTransfer = I2CMaster_I2C_CMPLT_ANY_TRANSFER; /* Pass control to Slave */
                        }
                        ...
                        */
                        else if((!I2CMaster_CHECK_INTR_SLAVE_MASKED(I2CMaster_INTR_SLAVE_I2C_BUS_ERROR))
                               && I2CMaster_CHECK_INTR_MASTER_MASKED(I2CMaster_INTR_MASTER_I2C_ARB_LOST))
                        {
                            endTransfer = I2CMaster_I2C_CMPLT_ANY_TRANSFER; /* Pass control to Slave */
                        }
                        else
                        ...
                        {
                            I2CMaster_CLEAR_TX_FIFO; /* Shifter keeps address, clear it */

                            I2CMaster_DISABLE_MASTER_AUTO_DATA_ACK; /* In case of reading disable autoACK */
                            ...
                            I2CMaster_SetRxInterruptMode(I2CMaster_NO_INTR_SOURCES);
                        ...
                        else
                        {
                            ...
                            I2CMaster_state = I2CMaster_I2C_FSM_EXIT_IDLE;
                        }
                    }
                    else
                    {
    ...
    else /* (I2CMaster_CHECK_I2C_FSM_SLAVE) */
    {
        #if(I2CMaster_I2C_SLAVE)
        {
            /* INTR_SLAVE_I2C_BUS_ERROR or I2CMaster_INTR_SLAVE_I2C_ARB_LOST:
            * A Misplaced Start or Stop condition occurred on the bus: set a flag
            * to notify an error condition.
            */
            if(I2CMaster_CHECK_INTR_SLAVE_MASKED(I2CMaster_INTR_SLAVE_I2C_BUS_ERROR |
                                                        I2CMaster_INTR_SLAVE_I2C_ARB_LOST))
            {
                if(I2CMaster_CHECK_I2C_FSM_RD)
                {
                    /* TX direction: master reads from slave */
                    I2CMaster_slStatus &= (uint8) ~I2CMaster_I2C_SSTAT_RD_BUSY;
                    I2CMaster_slStatus |= (uint8) (I2CMaster_I2C_SSTAT_RD_ERR |
                                                          I2CMaster_I2C_SSTAT_RD_CMPLT);
                }
                else
                {
                    /* RX direction: master writes into slave */
                    I2CMaster_slStatus &= (uint8) ~I2CMaster_I2C_SSTAT_WR_BUSY;
                    I2CMaster_slStatus |= (uint8) (I2CMaster_I2C_SSTAT_WR_ERR |
                                                          I2CMaster_I2C_SSTAT_WR_CMPLT);
                }
    ...
    */
    ...
    if(I2CMaster_CHECK_I2C_FSM_MASTER)
    {
        #if(I2CMaster_I2C_MASTER)
        {
            ...
            */
            if(I2CMaster_CHECK_INTR_MASTER_MASKED(I2CMaster_INTR_MASTER_I2C_STOP))
            {
            ...
            {
                if(I2CMaster_CHECK_I2C_FSM_ADDR) /* Address stage */
                {
                    /* INTR_MASTER_I2C_NACK:
                    * The master sent an address but it was NACKed by the slave. Complete transaction.
                    */
                    if(I2CMaster_CHECK_INTR_MASTER_MASKED(I2CMaster_INTR_MASTER_I2C_NACK))
                    {
                        I2CMaster_ClearMasterInterruptSource(I2CMaster_INTR_MASTER_I2C_NACK);

                        I2CMaster_mstrStatus |= (uint16) (I2CMaster_I2C_MSTAT_ERR_XFER |
                                                                 I2CMaster_I2C_MSTAT_ERR_ADDR_NAK);
                    ...
                    */
                    ...
                    {
                        if(I2CMaster_CHECK_I2C_FSM_RD) /* Reading */
                        {
                        ...
                        else /* Writing */
                        {
                            ...
                            if(0u != I2CMaster_mstrWrBufSize)
                            {
                                /* Enable INTR.TX_EMPTY if there is data to transmit */
                                I2CMaster_SetTxInterruptMode(I2CMaster_INTR_TX_EMPTY);
                            }
                        }
                    }
                }

                if(I2CMaster_CHECK_I2C_FSM_DATA) /* Data phase */
                {
                    if(I2CMaster_CHECK_I2C_FSM_RD) /* Reading */
                    {
                        /* INTR_RX_FULL:
                        ...
                        */
                        if(I2CMaster_CHECK_INTR_RX_MASKED(I2CMaster_INTR_RX_FULL))
                        {
                            /* Calculate difference */
                            diffCount =  I2CMaster_mstrRdBufSize -
                                        (I2CMaster_mstrRdBufIndex + I2CMaster_GET_RX_FIFO_ENTRIES);

                            /* Proceed transaction or end it when RX FIFO becomes FULL again */
                            if(diffCount > I2CMaster_I2C_FIFO_SIZE)
                            {
                                diffCount = I2CMaster_I2C_FIFO_SIZE;
                            }
                            else
                            {
                                if(0u == diffCount)
                                {
                                    I2CMaster_DISABLE_MASTER_AUTO_DATA_ACK;
                            ...
                            {
                                I2CMaster_mstrRdBufPtr[I2CMaster_mstrRdBufIndex] = (uint8)
                                                                                        I2CMaster_RX_FIFO_RD_REG;
                        ...
                        */
                        else if(I2CMaster_CHECK_INTR_RX_MASKED(I2CMaster_INTR_RX_NOT_EMPTY))
                        {
                            /* Put data in component buffer */
                            I2CMaster_mstrRdBufPtr[I2CMaster_mstrRdBufIndex] = (uint8) I2CMaster_RX_FIFO_RD_REG;
                            I2CMaster_mstrRdBufIndex++;
                        ...
                        I2CMaster_ClearRxInterruptSource(I2CMaster_INTR_RX_ALL);
                    }
                    else /* Writing */
                    {
                        /* INTR_MASTER_I2C_NACK :
                        ...
                        */
                        if(I2CMaster_CHECK_INTR_MASTER_MASKED(I2CMaster_INTR_MASTER_I2C_NACK))
                        {
                            ...
                            I2CMaster_mstrWrBufIndexTmp -= (I2CMaster_GET_TX_FIFO_ENTRIES +
                                                                   I2CMaster_GET_TX_FIFO_SR_VALID);

                            /* Update number of transferred bytes */
                            I2CMaster_mstrWrBufIndex = I2CMaster_mstrWrBufIndexTmp;

                            I2CMaster_mstrStatus |= (uint16) (I2CMaster_I2C_MSTAT_ERR_XFER |
                                                                     I2CMaster_I2C_MSTAT_ERR_SHORT_XFER);

                            I2CMaster_CLEAR_TX_FIFO;
                        ...
                        */
                        else if(I2CMaster_CHECK_INTR_TX_MASKED(I2CMaster_INTR_TX_EMPTY))
                        {
                            while(I2CMaster_I2C_FIFO_SIZE != I2CMaster_GET_TX_FIFO_ENTRIES)
                            {
                                ...
                                */
                                if(I2CMaster_mstrWrBufIndexTmp < I2CMaster_mstrWrBufSize)
                                {
                                    ...
                                    */
                                    if(I2CMaster_mstrWrBufIndexTmp == (I2CMaster_mstrWrBufSize - 1u))
                                    {
                                    ...
                                    I2CMaster_TX_FIFO_WR_REG = (uint32) I2CMaster_mstrWrBufPtr[I2CMaster_mstrWrBufIndexTmp];
                                    I2CMaster_mstrWrBufIndexTmp++;
                                }
                                else
                                {
                            ...
                            I2CMaster_ClearTxInterruptSource(I2CMaster_INTR_TX_ALL);
                        #else
                            I2CMaster_ClearTxInterruptSource(I2CMaster_INTR_TX_EMPTY);
                        #endif /* (I2CMaster_CY_SCBIP_V0) */
                        }
                        /* INTR_TX_UNDERFLOW:
                        ...
                        */
                        else if(I2CMaster_CHECK_INTR_TX_MASKED(I2CMaster_INTR_TX_UNDERFLOW))
                        {
                            /* Update number of transferred bytes */
                            I2CMaster_mstrWrBufIndex = I2CMaster_mstrWrBufIndexTmp;
                ...
                if(0u != endTransfer) /* Complete transfer */
                {
                    ...
                    if(I2CMaster_CHECK_I2C_MODE_NO_STOP(I2CMaster_mstrControl))
                    {
        ...
        } /* (I2CMaster_I2C_MASTER) */
        #endif

    } /* (I2CMaster_CHECK_I2C_FSM_MASTER) */


    /* FSM Slave */
    else if(I2CMaster_CHECK_I2C_FSM_SLAVE)
    {
        #if(I2CMaster_I2C_SLAVE)
        {
            /* INTR_SLAVE_NACK:
            ...
            */
            if(I2CMaster_CHECK_INTR_SLAVE_MASKED(I2CMaster_INTR_SLAVE_I2C_NACK))
            {
                I2CMaster_ClearSlaveInterruptSource(I2CMaster_INTR_SLAVE_I2C_NACK);

                /* All entries that remain in TX FIFO are: FIFO Size + 1 (SHIFTER) */
                diffCount = (I2CMaster_GET_TX_FIFO_ENTRIES + I2CMaster_GET_TX_FIFO_SR_VALID);

                if(I2CMaster_slOverFlowCount > diffCount) /* Overflow */
                {
                    I2CMaster_slStatus |= (uint8) I2CMaster_I2C_SSTAT_RD_OVFL;
                }
                else /* No Overflow */
                {
                    /* Roll-back temporary index */
                    I2CMaster_slRdBufIndexTmp -= (diffCount - I2CMaster_slOverFlowCount);
                }

                /* Update slave of transferred bytes */
                I2CMaster_slRdBufIndex = I2CMaster_slRdBufIndexTmp;

                /* Clean-up TX FIFO */
                I2CMaster_SetTxInterruptMode(I2CMaster_NO_INTR_SOURCES);
                I2CMaster_slOverFlowCount = 0u;
                I2CMaster_CLEAR_TX_FIFO;

                /* Complete master reading */
                I2CMaster_slStatus &= (uint8) ~I2CMaster_I2C_SSTAT_RD_BUSY;
                I2CMaster_slStatus |= (uint8)  I2CMaster_I2C_SSTAT_RD_CMPLT;
                I2CMaster_state     =  I2CMaster_I2C_FSM_IDLE;
            }
            ...
            */
            if(I2CMaster_CHECK_INTR_SLAVE_MASKED(I2CMaster_INTR_SLAVE_I2C_WRITE_STOP))
            {
                I2CMaster_ClearSlaveInterruptSource(I2CMaster_INTR_SLAVE_I2C_WRITE_STOP);
                ...
                * I2C_ADDR_MATCH.
                */
                if (0u != (I2CMaster_I2C_CTRL_REG & I2CMaster_I2C_CTRL_S_READY_DATA_ACK))
                {
                    while(0u != I2CMaster_GET_RX_FIFO_ENTRIES)
                    {
                        #if(I2CMaster_CHECK_I2C_ACCEPT_ADDRESS)
                        {
                            if((1u == I2CMaster_GET_RX_FIFO_ENTRIES) &&
                               (I2CMaster_CHECK_INTR_SLAVE_MASKED(I2CMaster_INTR_SLAVE_I2C_ADDR_MATCH)))
                            {
                        ...
                        /* Put data in component buffer */
                        I2CMaster_slWrBufPtr[I2CMaster_slWrBufIndex] = (uint8) I2CMaster_RX_FIFO_RD_REG;
                        I2CMaster_slWrBufIndex++;
                    }

                    I2CMaster_DISABLE_SLAVE_AUTO_DATA;
                }

                if(I2CMaster_CHECK_INTR_RX(I2CMaster_INTR_RX_OVERFLOW))
                {
                    I2CMaster_slStatus |= (uint8) I2CMaster_I2C_SSTAT_WR_OVFL;
                }
                ...
                I2CMaster_slStatus &= (uint8) ~I2CMaster_I2C_SSTAT_WR_BUSY;
                I2CMaster_slStatus |= (uint8)  I2CMaster_I2C_SSTAT_WR_CMPLT;
                I2CMaster_state     =  I2CMaster_I2C_FSM_IDLE;
            }
            ...
            */
        #if (I2CMaster_I2C_CUSTOM_ADDRESS_HANDLER_CONST)
            if (I2CMaster_CHECK_INTR_SLAVE_MASKED(I2CMaster_INTR_SLAVE_I2C_ADDR_MATCH |
        ...
        #endif /* (I2CMaster_I2C_CUSTOM_ADDRESS_HANDLER_CONST) */
            {
                ...
                I2CMaster_ClearI2CExtClkInterruptSource(I2CMaster_INTR_I2C_EC_WAKE_UP);

                #if (I2CMaster_I2C_CUSTOM_ADDRESS_HANDLER)
                {
                    if (NULL != I2CMaster_customAddressHandler)
                    {
                        /* Call custom address handler */
                        response = I2CMaster_customAddressHandler();
                    }
                    else
                    {
                        /* Read address from the RX FIFO. If there is no address underflow triggers but
                        * component does not use that source. */
                        (void) I2CMaster_RX_FIFO_RD_REG;
                        response = I2CMaster_I2C_ACK_ADDR;
                    }
            ...
            #if (I2CMaster_I2C_CUSTOM_ADDRESS_HANDLER_CONST)
                if (response == I2CMaster_I2C_NAK_ADDR)
                {
                #if (!I2CMaster_CY_SCBIP_V0)
                    /* Disable write stop interrupt source as it triggers after address was NACKed. Ticket ID#156094 */
                    I2CMaster_DISABLE_INTR_SLAVE(I2CMaster_INTR_SLAVE_I2C_WRITE_STOP);
                #endif /* (!I2CMaster_CY_SCBIP_V0) */

                    /* Clear address match and stop history */
                    I2CMaster_ClearSlaveInterruptSource(I2CMaster_INTR_SLAVE_ALL);

                    /* ACK the address byte */
                    I2CMaster_I2C_SLAVE_GENERATE_NACK;
                }
                else
            #endif /* (I2CMaster_I2C_CUSTOM_ADDRESS_HANDLER_CONST) */
                {
                    if(I2CMaster_CHECK_I2C_STATUS(I2CMaster_I2C_STATUS_S_READ))
                    /* TX direction: master reads from slave */
                    {
                        I2CMaster_SetTxInterruptMode(I2CMaster_INTR_TX_EMPTY);

                        /* Set temporary index to address buffer clear from API */
                        I2CMaster_slRdBufIndexTmp = I2CMaster_slRdBufIndex;

                        /* Start master reading */
                        I2CMaster_slStatus |= (uint8) I2CMaster_I2C_SSTAT_RD_BUSY;
                        I2CMaster_state     = I2CMaster_I2C_FSM_SL_RD;
                    }
                    else
                    /* RX direction: master writes into slave */
                    {
                        /* Calculate available buffer size */
                        diffCount = (I2CMaster_slWrBufSize - I2CMaster_slWrBufIndex);
                        ...
                        /* Receive data: into RX FIFO */
                        {
                            if(diffCount == I2CMaster_I2C_FIFO_SIZE)
                            {
                                ...
                                I2CMaster_ENABLE_SLAVE_AUTO_DATA;
                            }
                            else
                            {
                                /* Stretch clock when RX FIFO becomes FULL */
                                I2CMaster_ENABLE_SLAVE_AUTO_DATA_ACK;
                        ...
                        /* Start master reading */
                        I2CMaster_slStatus |= (uint8) I2CMaster_I2C_SSTAT_WR_BUSY;
                        I2CMaster_state     = I2CMaster_I2C_FSM_SL_WR;
                    }
                    ...
                    I2CMaster_ENABLE_INTR_SLAVE(I2CMaster_INTR_SLAVE_I2C_WRITE_STOP);
                #endif /* (!I2CMaster_CY_SCBIP_V0) */

                    /* ACK the address byte */
                    I2CMaster_I2C_SLAVE_GENERATE_ACK;
                }
            }
            ...
            */
            if(I2CMaster_CHECK_INTR_RX_MASKED(I2CMaster_INTR_RX_FULL))
            {
                ...
                diffCount =  I2CMaster_slWrBufSize -
                            (I2CMaster_slWrBufIndex + I2CMaster_I2C_FIFO_SIZE);

                if(diffCount > I2CMaster_I2C_FIFO_SIZE) /* Proceed transaction */
                {
                    diffCount   = I2CMaster_I2C_FIFO_SIZE;
                    endTransfer = 0u;  /* Continue active transfer */
                }
                ...
                {
                    I2CMaster_ENABLE_SLAVE_AUTO_DATA_NACK;
                ...
                I2CMaster_ClearRxInterruptSource(I2CMaster_INTR_RX_FULL);
            }
            ...
            */
            ...
            {
                diffCount = I2CMaster_RX_FIFO_RD_REG;
                ...
                if(I2CMaster_slWrBufIndex < I2CMaster_slWrBufSize)
                {
                    ...
                    I2CMaster_slWrBufPtr[I2CMaster_slWrBufIndex] = (uint8) diffCount;
                    I2CMaster_slWrBufIndex++;
                }
            ...
            */
            if(I2CMaster_CHECK_INTR_TX_MASKED(I2CMaster_INTR_TX_EMPTY))
            {
                while(I2CMaster_I2C_FIFO_SIZE != I2CMaster_GET_TX_FIFO_ENTRIES)
                {
                    ...
                    */
                    if(I2CMaster_slRdBufIndexTmp < I2CMaster_slRdBufSize)
                    /* Data from buffer */
                    {
                        I2CMaster_TX_FIFO_WR_REG = (uint32) I2CMaster_slRdBufPtr[I2CMaster_slRdBufIndexTmp];
                        I2CMaster_slRdBufIndexTmp++;
                    }
                    else
                    /* Probably Overflow */
                    {
                        I2CMaster_TX_FIFO_WR_REG = I2CMaster_I2C_SLAVE_OVFL_RETURN;

                        if(I2CMaster_slOverFlowCount <= I2CMaster_I2C_TX_OVERFLOW_COUNT)
                        {
                            /* Get counter in range of overflow. */
                            I2CMaster_slOverFlowCount++;
                        }
                    }
                }
    ...
    */
    ...
    {
        I2CMaster_CTRL_REG &= (uint32) ~I2CMaster_CTRL_ENABLED; /* Disable scb IP */

        I2CMaster_state = I2CMaster_I2C_FSM_IDLE;

        I2CMaster_DISABLE_SLAVE_AUTO_DATA;
        I2CMaster_DISABLE_MASTER_AUTO_DATA;
        ...
        I2CMaster_ClearTxInterruptSource(I2CMaster_INTR_RX_ALL);
        I2CMaster_ClearRxInterruptSource(I2CMaster_INTR_TX_ALL);
        I2CMaster_ClearSlaveInterruptSource(I2CMaster_INTR_SLAVE_ALL);
        I2CMaster_ClearMasterInterruptSource(I2CMaster_INTR_MASTER_ALL);
    #endif /* (I2CMaster_CY_SCBIP_V0) */

        I2CMaster_CTRL_REG |= (uint32) I2CMaster_CTRL_ENABLED;  /* Enable scb IP */
    }

#ifdef I2CMaster_I2C_ISR_EXIT_CALLBACK
    I2CMaster_I2C_ISR_ExitCallback();
#endif /* I2CMaster_I2C_ISR_EXIT_CALLBACK */
