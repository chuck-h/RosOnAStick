...
/* Master variables */
volatile uint16 I2CMaster_mstrStatus;      /* Master Status byte  */
...
volatile uint32  I2CMaster_mstrRdBufSize;  /* Master Read buffer size       */
volatile uint32  I2CMaster_mstrRdBufIndex; /* Master Read buffer Index      */

/* Transmit buffer variables */
volatile uint8 * I2CMaster_mstrWrBufPtr;   /* Pointer to Master Write buffer */
volatile uint32  I2CMaster_mstrWrBufSize;  /* Master Write buffer size       */
volatile uint32  I2CMaster_mstrWrBufIndex; /* Master Write buffer Index      */
volatile uint32  I2CMaster_mstrWrBufIndexTmp; /* Master Write buffer Index Tmp */

#if (!I2CMaster_CY_SCBIP_V0 && \
    I2CMaster_I2C_MULTI_MASTER_SLAVE_CONST && I2CMaster_I2C_WAKE_ENABLE_CONST)
    static void I2CMaster_I2CMasterDisableEcAm(void);
#endif /* (!I2CMaster_CY_SCBIP_V0) */


/*******************************************************************************
* Function Name: I2CMaster_I2CMasterWriteBuf
****************************************************************************//**
*
...
*  Enables the I2C interrupt and clears I2CMaster_I2C_MSTAT_WR_CMPLT 
*  status.
*
...
*  - I2CMaster_I2C_MODE_COMPLETE_XFER - Perform complete transfer 
*    from Start to Stop.
*  - I2CMaster_I2C_MODE_REPEAT_START - Send Repeat Start instead 
*    of Start. A Stop is generated after transfer is completed unless 
*    NO_STOP is specified.
*  - I2CMaster_I2C_MODE_NO_STOP Execute transfer without a Stop.
*    The following transfer expected to perform ReStart.
*
* \return
*  Error status.
*  - I2CMaster_I2C_MSTR_NO_ERROR - Function complete without error. 
*    The master started the transfer.
*  - I2CMaster_I2C_MSTR_BUS_BUSY - Bus is busy. Nothing was sent on
*    the bus. The attempt has to be retried.
*  - I2CMaster_I2C_MSTR_NOT_READY - Master is not ready for to start
...
uint32 I2CMaster_I2CMasterWriteBuf(uint32 slaveAddress, uint8 * wrData, uint32 cnt, uint32 mode)
{
    ...
    errStatus = I2CMaster_I2C_MSTR_NOT_READY;

    if(NULL != wrData)  /* Check buffer pointer */
    {
        /* Check FSM state and bus before generating Start/ReStart condition */
        if(I2CMaster_CHECK_I2C_FSM_IDLE)
        {
            I2CMaster_DisableInt();  /* Lock from interruption */

            /* Check bus state */
            errStatus = I2CMaster_CHECK_I2C_STATUS(I2CMaster_I2C_STATUS_BUS_BUSY) ?
                            I2CMaster_I2C_MSTR_BUS_BUSY : I2CMaster_I2C_MSTR_NO_ERROR;
        }
        else if(I2CMaster_CHECK_I2C_FSM_HALT)
        {
            I2CMaster_mstrStatus &= (uint16) ~I2CMaster_I2C_MSTAT_XFER_HALT;
                              errStatus  = I2CMaster_I2C_MSTR_NO_ERROR;
        }
        else
        {
    ...
    if(I2CMaster_I2C_MSTR_NO_ERROR == errStatus) /* No error proceed */
    {
    #if (!I2CMaster_CY_SCBIP_V0 && \
        I2CMaster_I2C_MULTI_MASTER_SLAVE_CONST && I2CMaster_I2C_WAKE_ENABLE_CONST)
            I2CMaster_I2CMasterDisableEcAm();
    #endif /* (!I2CMaster_CY_SCBIP_V0) */

        /* Set up write transaction */
        I2CMaster_state = I2CMaster_I2C_FSM_MSTR_WR_ADDR;
        I2CMaster_mstrWrBufIndexTmp = 0u;
        I2CMaster_mstrWrBufIndex    = 0u;
        I2CMaster_mstrWrBufSize     = cnt;
        I2CMaster_mstrWrBufPtr      = (volatile uint8 *) wrData;
        I2CMaster_mstrControl       = (uint8) mode;

        slaveAddress = I2CMaster_GET_I2C_8BIT_ADDRESS(slaveAddress);

        I2CMaster_mstrStatus &= (uint16) ~I2CMaster_I2C_MSTAT_WR_CMPLT;

        I2CMaster_ClearMasterInterruptSource(I2CMaster_INTR_MASTER_ALL);
        I2CMaster_ClearTxInterruptSource(I2CMaster_INTR_TX_UNDERFLOW);

        /* The TX and RX FIFO have to be EMPTY */

        /* Enable interrupt source to catch when address is sent */
        I2CMaster_SetTxInterruptMode(I2CMaster_INTR_TX_UNDERFLOW);

        /* Generate Start or ReStart */
        if(I2CMaster_CHECK_I2C_MODE_RESTART(mode))
        {
            I2CMaster_I2C_MASTER_GENERATE_RESTART;
            I2CMaster_TX_FIFO_WR_REG = slaveAddress;
        }
        else
        {
            I2CMaster_TX_FIFO_WR_REG = slaveAddress;
            I2CMaster_I2C_MASTER_GENERATE_START;
        }
    }
...
/*******************************************************************************
* Function Name: I2CMaster_I2CMasterReadBuf
****************************************************************************//**
*
*  Automatically reads an entire buffer of data from a slave device.
*  Once the data transfer is initiated by this function, further data transfer
*  is handled by the included ISR.
*  Enables the I2C interrupt and clears I2CMaster_I2C_MSTAT_RD_CMPLT 
*  status.
*
*  \param slaveAddr: 7-bit slave address.
*  \param xferData: Pointer to buffer of data to be sent.
...
*  be ORed together. See I2CMaster_I2CMasterWriteBuf() 
*  function for constants.
*
* \return
*  Error status.See I2CMaster_I2CMasterWriteBuf() 
*  function for constants.
*
* \globalvars
*  I2CMaster_mstrStatus  - used to store current status of I2C Master.
...
*  I2CMaster_mstrControl - used to control master end of transaction with
*  or without the Stop generation.
*  I2CMaster_mstrRdBufPtr - used to store pointer to master read buffer.
*  I2CMaster_mstrRdBufIndex - used to current index within master read
*  buffer.
*  I2CMaster_mstrRdBufSize - used to store master read buffer size.
*
*******************************************************************************/
uint32 I2CMaster_I2CMasterReadBuf(uint32 slaveAddress, uint8 * rdData, uint32 cnt, uint32 mode)
{
    ...
    if(I2CMaster_I2C_MSTR_NO_ERROR == errStatus) /* No error proceed */
    {
        ...
        I2CMaster_state = I2CMaster_I2C_FSM_MSTR_RD_ADDR;
        I2CMaster_mstrRdBufIndex = 0u;
        I2CMaster_mstrRdBufSize  = cnt;
        I2CMaster_mstrRdBufPtr   = (volatile uint8 *) rdData;
        I2CMaster_mstrControl    = (uint8) mode;

        slaveAddress = (I2CMaster_GET_I2C_8BIT_ADDRESS(slaveAddress) | I2CMaster_I2C_READ_FLAG);

        I2CMaster_mstrStatus &= (uint16) ~I2CMaster_I2C_MSTAT_RD_CMPLT;
        ...
        /* Prepare reading */
        if(I2CMaster_mstrRdBufSize < I2CMaster_I2C_FIFO_SIZE)
        {
            /* Reading byte-by-byte */
            I2CMaster_SetRxInterruptMode(I2CMaster_INTR_RX_NOT_EMPTY);
        }
        else
        {
            /* Receive RX FIFO chunks */
            I2CMaster_ENABLE_MASTER_AUTO_DATA_ACK;
            I2CMaster_SetRxInterruptMode(I2CMaster_INTR_RX_FULL);
        }
...
/*******************************************************************************
* Function Name: I2CMaster_I2CMasterSendStart
****************************************************************************//**
*
...
*  - I2CMaster_I2C_WRITE_XFER_MODE - Set write direction for the 
*    following transfer.
*  - I2CMaster_I2C_READ_XFER_MODE - Set read direction for the 
*    following transfer.
*
* \return
*  Error status.
*   - I2CMaster_I2C_MSTR_NO_ERROR - Function complete without error.
*   - I2CMaster_I2C_MSTR_BUS_BUSY - Bus is busy. 
*     Nothing was sent on the bus. The attempt has to be retried.
*   - I2CMaster_I2C_MSTR_NOT_READY - Master is not ready for to 
*     start transfer.
...
*     Nothing was sent on the bus. The attempt has to be retried.
*   - I2CMaster_I2C_MSTR_ERR_LB_NAK - Error condition: Last byte was 
*     NAKed.
*   - I2CMaster_I2C_MSTR_ERR_ARB_LOST - Error condition: Master lost 
*     arbitration.
*   - I2CMaster_I2C_MSTR_ERR_BUS_ERR - Error condition: Master 
*     encountered a bus error. Bus error is misplaced start or stop detection.
*   - I2CMaster_I2C_MSTR_ERR_ABORT_START - Error condition: The start 
...
uint32 I2CMaster_I2CMasterSendStart(uint32 slaveAddress, uint32 bitRnW)
{
    ...
    if(I2CMaster_CHECK_I2C_FSM_IDLE)
    {
        ...
        if(I2CMaster_CHECK_I2C_STATUS(I2CMaster_I2C_STATUS_BUS_BUSY))
        {
            errStatus = I2CMaster_I2C_MSTR_BUS_BUSY;
        }
        else
        {
            I2CMaster_DisableInt();  /* Lock from interruption */
            ...
            {
                I2CMaster_state = I2CMaster_I2C_FSM_MSTR_WR_DATA;
            }
            else /* Read direction */
            {
                I2CMaster_state = I2CMaster_I2C_FSM_MSTR_RD_DATA;
                         slaveAddress |= I2CMaster_I2C_READ_FLAG;
            }

            /* TX and RX FIFO have to be EMPTY */

            I2CMaster_TX_FIFO_WR_REG = slaveAddress; /* Put address in TX FIFO */
            I2CMaster_ClearMasterInterruptSource(I2CMaster_INTR_MASTER_ALL);

            I2CMaster_I2C_MASTER_GENERATE_START;


            while(!I2CMaster_CHECK_INTR_MASTER(I2CMaster_INTR_MASTER_I2C_ACK      |
                                                      I2CMaster_INTR_MASTER_I2C_NACK     |
                                                      I2CMaster_INTR_MASTER_I2C_ARB_LOST |
            ...
            if(I2CMaster_CHECK_INTR_MASTER(I2CMaster_INTR_MASTER_I2C_ACK))
            {
                errStatus = I2CMaster_I2C_MSTR_NO_ERROR;
            }
            else if(I2CMaster_CHECK_INTR_MASTER(I2CMaster_INTR_MASTER_I2C_NACK))
            {
                errStatus = I2CMaster_I2C_MSTR_ERR_LB_NAK;
            }
            else if(I2CMaster_CHECK_INTR_MASTER(I2CMaster_INTR_MASTER_I2C_ARB_LOST))
            {
                I2CMaster_state = I2CMaster_I2C_FSM_IDLE;
                             errStatus = I2CMaster_I2C_MSTR_ERR_ARB_LOST;
                             resetIp   = I2CMaster_I2C_RESET_ERROR;
            }
            else /* I2CMaster_INTR_MASTER_I2C_BUS_ERROR set is else condition */
            {
                I2CMaster_state = I2CMaster_I2C_FSM_IDLE;
                             errStatus = I2CMaster_I2C_MSTR_ERR_BUS_ERR;
                             resetIp   = I2CMaster_I2C_RESET_ERROR;
            }

            I2CMaster_ClearMasterInterruptSource(I2CMaster_INTR_MASTER_I2C_ACK      |
                                                        I2CMaster_INTR_MASTER_I2C_NACK     |
                                                        I2CMaster_INTR_MASTER_I2C_ARB_LOST |
                                                        I2CMaster_INTR_MASTER_I2C_BUS_ERROR);
...
/*******************************************************************************
* Function Name: I2CMaster_I2CMasterSendRestart
****************************************************************************//**
*
...
*  See I2CMaster_I2CMasterSendStart() function for constants.
*
* \return
*  Error status. 
*  See I2CMaster_I2CMasterSendStart() function for constants.
...
uint32 I2CMaster_I2CMasterSendRestart(uint32 slaveAddress, uint32 bitRnW)
{
    ...
    if(I2CMaster_CHECK_I2C_MASTER_ACTIVE)
    {
        slaveAddress = I2CMaster_GET_I2C_8BIT_ADDRESS(slaveAddress);
        ...
        */
        ...
        while(I2CMaster_CHECK_I2C_MASTER_CMD(I2CMaster_I2C_MASTER_CMD_M_START))
        {
...
/*******************************************************************************
* Function Name: I2CMaster_I2CMasterSendStop
****************************************************************************//**
*
...
uint32 I2CMaster_I2CMasterSendStop(void)
{
    ...
    if(I2CMaster_CHECK_I2C_MASTER_ACTIVE)
    {
        ...
        */
        I2CMaster_I2C_MASTER_GENERATE_STOP;

        while(!I2CMaster_CHECK_INTR_MASTER(I2CMaster_INTR_MASTER_I2C_STOP     |
                                                  I2CMaster_INTR_MASTER_I2C_ARB_LOST |
                                                  I2CMaster_INTR_MASTER_I2C_BUS_ERROR))
        {
        ...
        if(I2CMaster_CHECK_INTR_MASTER(I2CMaster_INTR_MASTER_I2C_STOP))
        {
            errStatus = I2CMaster_I2C_MSTR_NO_ERROR;
        }
        else if(I2CMaster_CHECK_INTR_MASTER(I2CMaster_INTR_MASTER_I2C_ARB_LOST))
        ...
        else /* I2CMaster_INTR_MASTER_I2C_BUS_ERROR is set */
        {
            errStatus = I2CMaster_I2C_MSTR_ERR_BUS_ERR;
            resetIp   = I2CMaster_I2C_RESET_ERROR;
        }

        I2CMaster_ClearMasterInterruptSource(I2CMaster_INTR_MASTER_I2C_STOP     |
                                                    I2CMaster_INTR_MASTER_I2C_ARB_LOST |
                                                    I2CMaster_INTR_MASTER_I2C_BUS_ERROR);
...
/*******************************************************************************
* Function Name: I2CMaster_I2CMasterWriteByte
****************************************************************************//**
*
...
uint32 I2CMaster_I2CMasterWriteByte(uint32 theByte)
{
    ...
    if(I2CMaster_CHECK_I2C_MASTER_ACTIVE)
    {
        I2CMaster_TX_FIFO_WR_REG = theByte;
        ...
        if(I2CMaster_CHECK_INTR_MASTER(I2CMaster_INTR_MASTER_I2C_ACK))
        {
            I2CMaster_state = I2CMaster_I2C_FSM_MSTR_HALT;
                         errStatus = I2CMaster_I2C_MSTR_NO_ERROR;
        }
        else if(I2CMaster_CHECK_INTR_MASTER(I2CMaster_INTR_MASTER_I2C_NACK))
        {
        ...
        else /* I2CMaster_INTR_MASTER_I2C_BUS_ERROR set is */
        {
            I2CMaster_state = I2CMaster_I2C_FSM_IDLE;
                         errStatus = I2CMaster_I2C_MSTR_ERR_BUS_ERR;
                         resetIp   = I2CMaster_I2C_RESET_ERROR;
...
/*******************************************************************************
* Function Name: I2CMaster_I2CMasterReadByte
****************************************************************************//**
*
...
*  I2CMaster_I2CMasterSendStop() or 
*  I2CMaster_I2CMasterSendRestart() function appropriately.
*  This function is blocking. It does not return until a byte is received or 
*  an error occurs.
*
* \param ackNack: Response to received byte.
*  - I2CMaster_I2C_ACK_DATA - Generates ACK. 
*     The master notifies slave that transfer continues.
*  - I2CMaster_I2C_NAK_DATA - Prepares to generate NAK.
...
uint32 I2CMaster_I2CMasterReadByte(uint32 ackNack)
{
    ...
    theByte = I2CMaster_I2C_INVALID_BYTE;

    /* Check FSM state before read byte */
    if(I2CMaster_CHECK_I2C_MASTER_ACTIVE)
    {
        while((!I2CMaster_CHECK_INTR_RX(I2CMaster_INTR_RX_NOT_EMPTY)) &&
              (!I2CMaster_CHECK_INTR_MASTER(I2CMaster_INTR_MASTER_I2C_ARB_LOST |
                                                  I2CMaster_INTR_MASTER_I2C_BUS_ERROR)))
        {
        ...
        if(I2CMaster_CHECK_INTR_RX(I2CMaster_INTR_RX_NOT_EMPTY))
        {
            theByte = I2CMaster_RX_FIFO_RD_REG;

            I2CMaster_ClearRxInterruptSource(I2CMaster_INTR_RX_NOT_EMPTY);

            if(0u == ackNack)
            {
                I2CMaster_I2C_MASTER_GENERATE_ACK;
            }
            else
            {
...
/*******************************************************************************
* Function Name: I2CMaster_I2CMasterGetReadBufSize
****************************************************************************//**
*
*  Returns the number of bytes that has been transferred with an
*  I2CMaster_I2CMasterReadBuf() function.
*
* \return
...
*  I2CMaster_I2C_MSTAT_ERR_ARB_LOST or
*  I2CMaster_I2C_MSTAT_ERR_BUS_ERROR occurred while read transfer.
*
* \globalvars
*  I2CMaster_mstrRdBufIndex - used to current index within master read
*  buffer.
...
/*******************************************************************************
* Function Name: I2CMaster_I2CMasterGetWriteBufSize
****************************************************************************//**
*
...
/*******************************************************************************
* Function Name: I2CMaster_I2CMasterClearReadBuf
****************************************************************************//**
*
...
/*******************************************************************************
* Function Name: I2CMaster_I2CMasterClearWriteBuf
****************************************************************************//**
*
...
/*******************************************************************************
* Function Name: I2CMaster_I2CMasterStatus
****************************************************************************//**
*
...
*  - I2CMaster_I2C_MSTAT_RD_CMPLT - Read transfer complete.
*    The error condition status bits must be checked to ensure that 
*    read transfer was completed successfully.
*  - I2CMaster_I2C_MSTAT_WR_CMPLT - Write transfer complete.
*    The error condition status bits must be checked to ensure that write 
*    transfer was completed successfully.
*  - I2CMaster_I2C_MSTAT_XFER_INP - Transfer in progress.
*  - I2CMaster_I2C_MSTAT_XFER_HALT - Transfer has been halted. 
*    The I2C bus is waiting for ReStart or Stop condition generation.
*  - I2CMaster_I2C_MSTAT_ERR_SHORT_XFER - Error condition: Write 
*    transfer completed before all bytes were transferred. The slave NAKed 
*    the byte which was expected to be ACKed.
*  - I2CMaster_I2C_MSTAT_ERR_ADDR_NAK - Error condition: Slave did 
*    not acknowledge address.
*  - I2CMaster_I2C_MSTAT_ERR_ARB_LOST - Error condition: Master lost 
*    arbitration during communications with slave.
*  - I2CMaster_I2C_MSTAT_ERR_BUS_ERROR - Error condition: bus error 
*    occurred during master transfer due to misplaced Start or Stop 
*    condition on the bus.
*  - I2CMaster_I2C_MSTAT_ERR_ABORT_XFER - Error condition: Slave was 
...
*  - I2CMaster_I2C_MSTAT_ERR_XFER - Error condition: This is the 
...
uint32 I2CMaster_I2CMasterStatus(void)
{
    ...
    status = (uint32) I2CMaster_mstrStatus;

    if (I2CMaster_CHECK_I2C_MASTER_ACTIVE)
    {
        /* Add status of master pending transaction: MSTAT_XFER_INP */
        status |= (uint32) I2CMaster_I2C_MSTAT_XFER_INP;
    }
...
/*******************************************************************************
* Function Name: I2CMaster_I2CMasterClearStatus
****************************************************************************//**
*
*  Clears all status flags and returns the master status.
*
* \return
*  Current status of master. See the I2CMaster_I2CMasterStatus() 
*  function for constants.
*
* \globalvars
*  I2CMaster_mstrStatus - used to store current status of I2C Master.
...
uint32 I2CMaster_I2CMasterClearStatus(void)
{
    ...
    status = (uint32) I2CMaster_mstrStatus;
    I2CMaster_mstrStatus = I2CMaster_I2C_MSTAT_CLEAR;
...
/*******************************************************************************
* Function Name: I2CMaster_I2CReStartGeneration
****************************************************************************//**
*
...
void I2CMaster_I2CReStartGeneration(void)
{
    ...
    */
    uint32 status = I2CMaster_I2C_STATUS_REG;

    while(I2CMaster_WAIT_SDA_SET_HIGH)
    {
        /* Wait when slave release SDA line: SCL tHIGH is complete */
    }

    /* Prepare DR register to drive SCL line */
    I2CMaster_SET_I2C_SCL_DR(I2CMaster_I2C_SCL_LOW);

    /* Switch HSIOM to GPIO: SCL goes low */
    I2CMaster_SET_I2C_SCL_HSIOM_SEL(I2CMaster_HSIOM_GPIO_SEL);

    /* Disable SCB block */
    I2CMaster_CTRL_REG &= (uint32) ~I2CMaster_CTRL_ENABLED;

    if(0u != (status & I2CMaster_I2C_STATUS_M_READ))
    {
        /* Generate NACK use GPIO functionality */
        I2CMaster_SET_I2C_SCL_DR(I2CMaster_I2C_SCL_LOW);
        CyDelayUs(I2CMaster_I2C_TLOW_TIME); /* Count tLOW */

        I2CMaster_SET_I2C_SCL_DR(I2CMaster_I2C_SCL_HIGH);
        while(I2CMaster_WAIT_SCL_SET_HIGH)
        {
            /* Wait until slave releases SCL in case if it stretches */
        }
        CyDelayUs(I2CMaster_I2C_THIGH_TIME); /* Count tHIGH */
    }
    ...
    I2CMaster_I2C_MASTER_CMD_REG = I2CMaster_I2C_MASTER_CMD_M_START;

    /* Enable SCB block */
    I2CMaster_CTRL_REG |= (uint32) I2CMaster_CTRL_ENABLED;

    /* Switch HSIOM to I2C: */
    I2CMaster_SET_I2C_SCL_HSIOM_SEL(I2CMaster_HSIOM_I2C_SEL);
    ...
    cmd  = I2CMaster_I2C_MASTER_CMD_M_START;
    cmd |= I2CMaster_CHECK_I2C_STATUS(I2CMaster_I2C_STATUS_M_READ) ?
                (I2CMaster_I2C_MASTER_CMD_M_NACK) : (0u);

    I2CMaster_I2C_MASTER_CMD_REG = cmd;
    ...
    I2CMaster_I2C_MULTI_MASTER_SLAVE_CONST && I2CMaster_I2C_WAKE_ENABLE_CONST)
    /*******************************************************************************
    * Function Name: I2CMaster_I2CMasterDisableEcAm
    ****************************************************************************//**
    *
    ...
    static void I2CMaster_I2CMasterDisableEcAm(void)
    {
        ...
        * This applicable only for Multi-Master-Slave with wakeup enabled. Ticket ID#192742 */
        if (0u != (I2CMaster_CTRL_REG & I2CMaster_CTRL_EC_AM_MODE))
        {
            ...
            I2CMaster_Stop();
            I2CMaster_CTRL_REG &= (uint32) ~I2CMaster_CTRL_EC_AM_MODE;
            I2CMaster_Enable();
        }
    }
#endif /* (!I2CMaster_CY_SCBIP_V0) */
