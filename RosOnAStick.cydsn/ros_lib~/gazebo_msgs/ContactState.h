...
namespace gazebo_msgs
{
  ...
  class ContactState : public ros::Msg
  {
    ...
    virtual int deserialize(unsigned char *inbuffer)
    {
      ...
      offset += 3;
      for( uint8_t i = 0; i < wrenches_lengthT; i++){
      offset += this->st_wrenches.deserialize(inbuffer + offset);
      if (i < wrenches_length) {
      ...
      offset += 3;
      for( uint8_t i = 0; i < contact_positions_lengthT; i++){
      offset += this->st_contact_positions.deserialize(inbuffer + offset);
      if (i < contact_positions_length) {
      ...
      offset += 3;
      for( uint8_t i = 0; i < contact_normals_lengthT; i++){
      offset += this->st_contact_normals.deserialize(inbuffer + offset);
      if (i < contact_normals_length) {
      ...
      offset += 3;
      for( uint8_t i = 0; i < depths_lengthT; i++){
