...
static const char GETMODELPROPERTIES[] = "gazebo_msgs/GetModelProperties";
  ...
  class GetModelPropertiesResponse : public ros::Msg
  {
    ...
    virtual int deserialize(unsigned char *inbuffer)
    {
      ...
      offset += 3;
      for( uint8_t i = 0; i < body_names_lengthT; i++){
      ...
      offset += 3;
      for( uint8_t i = 0; i < geom_names_lengthT; i++){
      ...
      offset += 3;
      for( uint8_t i = 0; i < child_model_names_lengthT; i++){
