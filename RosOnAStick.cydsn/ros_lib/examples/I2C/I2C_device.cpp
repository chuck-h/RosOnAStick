...
void writeMessageCb(const std_msgs::UInt8MultiArray& msg) {
    I2CMaster_Start(); // for testing
    uint32_t status = 
      I2CMaster_I2CMasterWriteBuf(msg.data[0], msg.data+1, msg.data_length-1,
                                  I2CMaster_I2C_MODE_COMPLETE_XFER);
...
void readMessageCb(const std_msgs::UInt8MultiArray& msg) {
    ...
    max_readback_bytes = msg.data[msg.data_length-1];
    i2caddr = msg.data[0];
    if (msg.data_length > 2) { // do write address cycle
        uint32_t status = 
          I2CMaster_I2CMasterWriteBuf(i2caddr, msg.data+1, msg.data_length-2, I2CMaster_I2C_MODE_COMPLETE_XFER);
        if (status == I2CMaster_I2C_MSTR_NO_ERROR) {
...
void setup() {
    ...
    //I2CMaster_scl_SetDriveMode(I2CMaster_scl_DM_RES_UP);
    //I2CMaster_sda_SetDriveMode(I2CMaster_sda_DM_RES_UP);
    I2CMaster_Start();
	...
	nh.subscribe(cmd_sub_r);
}

void loop() {

    if(state==state_read_setup && I2CMaster_I2CMasterStatus()==I2CMaster_I2C_MSTAT_WR_CMPLT) {
        ...
        I2CMaster_I2CMasterClearReadBuf();
