    ...
    /* AutoTunning start */
        ...
        CapSense_lowLevelTuningDone = 1u;
			...
			for(curSensor = 0u; curSensor < CapSense_TOTAL_SCANSLOT_COUNT; curSensor++)
			{
				...
				for(curSensor = 0u; curSensor < CapSense_TOTAL_SCANSLOT_COUNT; curSensor++)
			    {
				    CapSense_compensationIDAC[curSensor] = (CapSense_modulationIDAC[curSensor] + 1u) / 2u;
					CapSense_modulationIDAC[curSensor] = CapSense_modulationIDAC[curSensor] / 2u;
			    }
...
__INLINE void CapSense_DisableSensor(uint32 sensor)
{
    ...
    *CapSense_prtSelTbl[port] &= ~(CapSense_CSD_HSIOM_MASK << pinHSIOMShift);
	    ...
	    if(sensorState == CapSense__SHIELD)
		{
			sensorState = CapSense__GROUND;
		}
	#endif /* (CapSense_IS_SHIELD_ENABLE == 0) */
	...
	if(sensorState != (uint32)CapSense__SHIELD)
	{
		...
		{   
		    /* Connected to Ground */
		    inactiveConnect = CapSense_SNS_GROUND_CONNECT;
		}
