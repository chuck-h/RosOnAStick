...
volatile uint32  I2CMaster_slWrBufSize;     /* Slave Receive buffer size  */
volatile uint32  I2CMaster_slWrBufIndex;    /* Slave Receive buffer Index */

/* Transmit buffer variables */
volatile uint8 * I2CMaster_slRdBufPtr;      /* Pointer to Transmit buffer  */
volatile uint32  I2CMaster_slRdBufSize;     /* Slave Transmit buffer size  */
volatile uint32  I2CMaster_slRdBufIndex;    /* Slave Transmit buffer Index */
volatile uint32  I2CMaster_slRdBufIndexTmp; /* Slave Transmit buffer Index Tmp */
volatile uint8   I2CMaster_slOverFlowCount; /* Slave Transmit Overflow counter */

#if (I2CMaster_I2C_CUSTOM_ADDRESS_HANDLER_CONST)

    uint32 (*I2CMaster_customAddressHandler)(void) = NULL;

    /*******************************************************************************
    * Function Name: I2CMaster_SetI2cAddressCustomInterruptHandler
    ****************************************************************************//**
    *
    ...
    *******************************************************************************/
    void I2CMaster_SetI2cAddressCustomInterruptHandler(uint32 (*func) (void))
    {
        ...
        I2CMaster_customAddressHandler = func;
    }
#endif /* (I2CMaster_I2C_CUSTOM_ADDRESS_HANDLER_CONST) */


/*******************************************************************************
* Function Name: I2CMaster_I2CSlaveStatus
****************************************************************************//**
*
...
*   - I2CMaster_I2C_SSTAT_RD_CMPLT - Slave read transfer complete. 
...
*   - I2CMaster_I2C_SSTAT_RD_BUSY - Slave read transfer is in progress. 
*     Set when master addresses slave with a read, cleared when 
*    I2CMaster_I2C_SSTAT_RD_CMPLT is set.
*   - I2CMaster_I2C_SSTAT_RD_OVFL - Master attempted to read more bytes
*     than are in buffer. Slave continually returns 0xFF byte in this case.
*   - I2CMaster_I2C_SSTAT_RD_ERR - Slave captured error on the bus during 
*     a read transfer. The sources of error are: misplaced Start or Stop 
*     condition or lost arbitration while slave drives SDA.
*   - I2CMaster_I2C_SSTAT_WR_CMPLT - Slave write transfer complete. 
...
*     successfully.
*   - I2CMaster_I2C_SSTAT_WR_BUSY - Slave write transfer is in progress. 
*     Set when the master addresses the slave with a write, cleared when 
*    I2CMaster_I2C_SSTAT_WR_CMPLT is set.
*   - I2CMaster_I2C_SSTAT_WR_OVFL - Master attempted to write past end of 
*     buffer. Further bytes are ignored.
*   - I2CMaster_I2C_SSTAT_WR_ERR - Slave captured error on the bus during
...
*     I2CMaster_I2C_SSTAT_WR_ERR is set. It is recommended to discard write 
...
{
    return((uint32) I2CMaster_slStatus);
}


/*******************************************************************************
* Function Name: I2CMaster_I2CSlaveClearReadStatus
****************************************************************************//**
*
...
*
* \sideeffect
*  This function does not clear I2CMaster_I2C_SSTAT_RD_BUSY.
*
* \globalvars
*  I2CMaster_slStatus  - used to store current status of I2C slave.
*
...
uint32 I2CMaster_I2CSlaveClearReadStatus(void)
{
    ...
    status = ((uint32) I2CMaster_slStatus & I2CMaster_I2C_SSTAT_RD_MASK);
    I2CMaster_slStatus &= (uint8) ~I2CMaster_I2C_SSTAT_RD_CLEAR;
...
/*******************************************************************************
* Function Name: I2CMaster_I2CSlaveClearWriteStatus
****************************************************************************//**
*
...
*
* \sideeffect
*  This function does not clear I2CMaster_I2C_SSTAT_WR_BUSY.*
*
* \globalvars
*  I2CMaster_slStatus  - used to store current status of I2C slave.
*
...
uint32 I2CMaster_I2CSlaveClearWriteStatus(void)
{
    ...
    /* Mask of transfer complete flag and Error status */
    status = ((uint32) I2CMaster_slStatus & I2CMaster_I2C_SSTAT_WR_MASK);
    I2CMaster_slStatus &= (uint8) ~I2CMaster_I2C_SSTAT_WR_CLEAR;
...
/*******************************************************************************
* Function Name: I2CMaster_I2CSlaveSetAddress
****************************************************************************//**
*
...
void I2CMaster_I2CSlaveSetAddress(uint32 address)
{
    ...
    matchReg = I2CMaster_RX_MATCH_REG;

    matchReg &= ((uint32) ~I2CMaster_RX_MATCH_ADDR_MASK); /* Clear address bits */
    matchReg |= ((uint32)  I2CMaster_GET_I2C_8BIT_ADDRESS(address));

    I2CMaster_RX_MATCH_REG = matchReg;
}


/*******************************************************************************
* Function Name: I2CMaster_I2CSlaveSetAddressMask
****************************************************************************//**
*
...
void I2CMaster_I2CSlaveSetAddressMask(uint32 addressMask)
{
    ...
    matchReg &= ((uint32) ~I2CMaster_RX_MATCH_MASK_MASK);
    matchReg |= ((uint32) (addressMask << I2CMaster_RX_MATCH_MASK_POS));
...
void I2CMaster_I2CSlaveInitReadBuf(uint8 * rdBuf, uint32 bufSize)
{
    ...
    if(NULL != rdBuf)
    {
        ...
        I2CMaster_slRdBufIndex    = 0u;                       /* Clear buffer index  */
        I2CMaster_slRdBufIndexTmp = 0u;                       /* Clear buffer index  */
...
/*******************************************************************************
* Function Name: I2CMaster_I2CSlaveInitWriteBuf
****************************************************************************//**
*
*  Sets the buffer pointer and size of the write buffer. This function also 
*  resets the transfer count returned with the 
*  I2CMaster_I2CSlaveGetWriteBufSize() function.
*
...
void I2CMaster_I2CSlaveInitWriteBuf(uint8 * wrBuf, uint32 bufSize)
{
    ...
    if(NULL != wrBuf)
    {
        ...
        I2CMaster_slWrBufIndex = 0u;                       /* Clear buffer index  */
...
/*******************************************************************************
* Function Name: I2CMaster_I2CSlaveGetReadBufSize
****************************************************************************//**
*
*  Returns the number of bytes read by the I2C master since an
*  I2CMaster_I2CSlaveInitReadBuf() or 
*  I2CMaster_I2CSlaveClearReadBuf() function was executed.
...
*
* \sideeffect
*  The returned value is not valid if I2CMaster_I2C_SSTAT_RD_ERR was 
...
/*******************************************************************************
* Function Name: I2CMaster_I2CSlaveGetWriteBufSize
****************************************************************************//**
*
*  Returns the number of bytes written by the I2C master since an
*  I2CMaster_I2CSlaveInitWriteBuf() or 
*  I2CMaster_I2CSlaveClearWriteBuf() function was executed.
...
*
* \sideeffect
*  The returned value is not valid if I2CMaster_I2C_SSTAT_WR_ERR was 
*  captured by the slave.
*
* \globalvars
*  I2CMaster_slWrBufIndex - used to store current index within slave
...
/*******************************************************************************
* Function Name: I2CMaster_I2CSlaveClearReadBuf
****************************************************************************//**
*
...
/*******************************************************************************
* Function Name: I2CMaster_I2CSlaveClearWriteBuf
****************************************************************************//**
*
